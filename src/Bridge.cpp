...
Bridge::Bridge(MainWindow *mamaWindow_) : QObject(NULL),
        running_status(0),
        data_expected(0),
        msg_data(),
        midiIn(NULL),
...
void Bridge::sendMidiMessage() {
  ...
  //printf("Message Data3: %s\n", msg_data.mid(4, msg_data[3]).data());
  msg_data.clear();
  data_expected = 0;
}
...
QString Bridge::describeMIDI(QByteArray &buf)
{
    ...
    int buf2 = (uint8_t) buf[2] << 7;
    int buf1 = (uint8_t) buf[1];
    double bufCon = buf1 | buf2;
    //AbstractFunction *midi_control1 = new FunctionMidi(2);

    std::cout << bufCon << std::endl;

    double value = ((float) ((int) buf[1]) / ((int) buf[2]));

    printf("Value?: %f\n", value);
    printf("bufCon: %f\n", bufCon / 16000.0f);


    if (1 > (bufCon / 16000.0f))
    {
        //mamaWindow->midiSignal(bufCon);
        static_cast<FunctionMidi*>(mamaWindow->midiControl)->midiVal(bufCon / 16000.0f);
    }

    std::cout << qdesc.toUtf8().constData() << std::endl;
